...
// 用于 prompt 模板（不是官方 Video Agent 模板）
// ============================================================================
...
/**
 * 轮询基础视频任务状态
 * 注意：基础视频 API 的成功响应返回 file_id（不是直接 video_url），需要再调 /v1/files/retrieve
 */
export async function pollBasicVideoTask(taskId: string): Promise<PollResult> {
  ...
  // 基础 API 成功后还需要拿 file_id 换下载 URL
